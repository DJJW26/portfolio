...
// --- API: resume download (serves the PDF generated by build_resume.py) ---
app.get('/api/resume', (req, res) => {
  const resumePath = path.join(__dirname, 'Divij-Jhunjhunwala-Resume.pdf');
  if (!fs.existsSync(resumePath)) {
    return res.status(404).json({ error: 'Resume file not found on server.' });
  }
  res.download(resumePath, 'Divij-Jhunjhunwala-Resume.pdf');
});
